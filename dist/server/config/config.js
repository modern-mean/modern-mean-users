...
function load() {
  return exports.config = config = {
    logs: {
      ...
      winston: {
        level: process.env.MEAN_USERS_WINSTON_LEVEL || 'info', //{ error: 0, warn: 1, info: 2, verbose: 3, debug: 4, silly: 5 }
        file: process.env.MEAN_USERS_WINSTON_FILE || './logs/users.log',
        console: process.env.MEAN_USERS_WINSTON_CONSOLE || 'true'
      }
    },
    mongoose: {
